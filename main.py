...
async def get_valid_session_and_channels():
    ...
    async with httpx.AsyncClient(timeout=15.0, follow_redirects=True, cookies=cookies, headers=headers) as client:
        try:
            ...
            # Сохраняем токен в куки клиента
            ...
            genres_map = {}
            try:
                genres_res = await client.get(f"{PORTAL_URL}?type=itv&action=get_genres&JsHttpRequest=1-xml{token_param}")
            ...
            try:
                ch_res = await client.get(f"{PORTAL_URL}?type=itv&action=get_all_channels&JsHttpRequest=1-xml{token_param}")
                ch_json = ch_res.json()
